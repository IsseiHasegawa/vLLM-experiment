...
and parallelism strategy. All numbers below come from those runs.
...
not reach: HTTP receipt and response, serialisation, tokenisation, the
hand-off from frontend to engine, and any wait before the request
...
it holds attention compute and framework overhead as well.
...
`verify_report_numbers.py` recomputes every claim made here from the raw
logs — it passes from a clean clone.